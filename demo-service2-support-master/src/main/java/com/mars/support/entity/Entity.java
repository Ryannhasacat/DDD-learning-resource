 ...
 * 
 */
package com.mars.support.entity;
...
public abstract class Entity<T extends Serializable> implements Serializable, Cloneable {
	...
	public String toString() {
		...
		for(int i=0; i<fields.length; i++) {
			...
			if("serialVersionUID".equals(name)||"this$0".equals(name)) {
				continue;
			}
	...
	private boolean isExclude(String field) {
		for(int i=0; i<exclude.length ;i++)
			if(exclude[i].equals(field)||"serialVersionUID".equals(field)) {
				return true;
			}
		return false;
	}
